...
WHERE `user_id` = '$user_id' AND `invoice_status` = 0 AND `com_name` = 'writer_inv' AND `inTrash` = 0 ORDER BY `date_invoice` DESC");
if(!$statment_inbox) die('qyr $statment_inbox'.$user_id);
...
<table id="manual-table" class="table table-hover">
            <thead class="hidden">
            	<tr>
                    <th>إسم المحل</th>
                    <th>corner</th>
          ...
          if($total_rows > 0 ){
            while($row = mysqli_fetch_assoc($statment_inbox)){
              ...
              echo '
                ...
                <td class="text-center">
                  <a href="#" id="'.$order_id.'" class="inTrashManual">
...
<script>
	...
	var table_manual = $('#manual-table').DataTable({
					"pageLength":12
		});
    ...
    on_click_inbox();
		...
		*/
		$('#searchBox').keyup(function() {
		   table_manual.fnFilter(this.value);
		});
...
function on_click_manual(){
	...
	// move to TRASH folder
		$(document).on('click','.inTrashManual',function(){
					var id = $(this).attr("id");
